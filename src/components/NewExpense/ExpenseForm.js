...
// });
  ...
  const dateChangeHandler = (event) => {
    ...
    const submitHandler = (event) => {
        ...
        setEnteredDate('');
        setEnteredTitle('');
        props.setReturnValue(props.defaultContent);
    };

    const cancelHandler = () => props.setReturnValue(props.defaultContent);
  ...
  return (
    <form onSubmit={submitHandler}>
      ...
      <div className="new-expense__actions">
        <button onClick = {cancelHandler} className="button-style">Cancel</button>
        <button type="submit" className="button-style">Add Expense</button>
      </div>
    </form>
  );
};
